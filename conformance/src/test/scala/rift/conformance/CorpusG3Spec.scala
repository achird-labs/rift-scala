...
/** G3 — replay over the **embedded** engine (engine-required, GUARDED).
  ...
  * `rift.bridge.RiftConnector.isEmbeddedAvailable` is checked BEFORE any engine layer is built, so
  * a bare JVM (no `rift-java-embedded` / `--enable-native-access`, e.g. the JDK 21 CI job or a
  * local JDK < 22) skips (never fails) and this spec's only job there is to COMPILE. On a JDK 22+
  * job the embedded jars are on the classpath (wired in `build.sbt`), so it replays every
  * `hasVerify` fixture the embedded lane can serve. The out-of-process transport is covered by
  * [[CorpusG3SpawnSpec]].
  *
