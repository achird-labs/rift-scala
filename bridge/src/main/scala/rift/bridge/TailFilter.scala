...
/** A server-side filter for the cursor request tail (DESIGN.md §5.3, D6), mirroring exactly what
  * the facade's `MatchClause` can express — `header`, `flowId`, `method` and `path` — and nothing
  * more. The engine advances the cursor past `match`-rejected entries, so a filtered tail never
  * re-scans the journal; that is the whole point of filtering server-side rather than client-side
  ...
  * tail (`rift.fs2.pipes.matching`, or a `collect`/`filter` over the ZIO stream). A `Space`-scoped
  * cursor remains a tracked upstream ask.
  */
enum TailFilter:
