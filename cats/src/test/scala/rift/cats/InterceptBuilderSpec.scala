...
class InterceptBuilderSpec extends FunSuite:
  ...
  // #120: the typed overload must take the same per-terminal replay path as the string form — the
  // fold is where a dropped clause would silently widen the rule (#82).
  test("forward(port) replays the clauses too"):
    ...
    runToNpe(
      handle
        ...
        .when(first)
        .when(second)
        .forward(Port.from(4545).toOption.getOrElse(fail("4545 is a valid port")))
    )
    assertEquals(fake.ruleCalls, 1)
    val sent = InterceptGate.facadePredicates(fake.lastBuilder)
    assertEquals(sent.size, (first.predicates ++ second.predicates).size)
