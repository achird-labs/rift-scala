      ...
      ZIO.scoped {
        withControl() { control =>
          for
            ...
            _ <- interceptApi.add(
              spi.InterceptRule
                .Serve("inline.example.com", spi.InterceptStub(status = 203, body = Some("inline")))
            )
      ...
      ZIO.scoped {
        for
          riftEnv <- Rift.embedded
            ...
            .build
          controlEnv <- (ZLayer.succeedEnvironment(riftEnv) >>> RiftScalaBackend.fromService).build
          ...
          imposters <- rift.imposters.mapError(e => spi.MockError.CommunicationError(e.getMessage))
        yield assertTrue(
          exit.isFailure,
