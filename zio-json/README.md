...
**Don't wildcard-import `rift.json` alongside `zio`.** This package is `rift.json.zio`, so
`import rift.json.*` binds the name `zio` to it and shadows the root `zio` package:

```scala
import rift.json.*          // binds `zio` -> rift.json.zio
import zio.json.JsonCodec   // error: value json is not a member of rift.json.zio
```

Import what you need (`import rift.json.Json`), or anchor the other side (`import _root_.zio.json.*`).

**`import rift.json.zio.given` also re-routes the primitives.** A given imported into lexical scope
outranks one found in a companion, so `JsonBody[String]`, `JsonBody[Int]` and friends resolve to
zio-json's codecs rather than the instances `model` ships. They encode identically — only the
`decode` failure messages change (zio-json's `(expected string)` instead of model's
`expected a JSON string, got: 1`). Import `rift.json.zio.jsonBody` alone if you would rather keep the
model's messages for primitives.
